...
func convertSDKAffinityGroup(sdkObject *ovirtsdk.AffinityGroup, o *oVirtClient) (AffinityGroup, error) {
	...
	if !ok {
		return nil, newFieldNotFound("affinity group", "name")
	}
	cluster, ok := sdkObject.Cluster()
	if !ok {
		return nil, newFieldNotFound("affinity group", "cluster")
	}
	clusterID, ok := cluster.Id()
	if !ok {
		return nil, newFieldNotFound("cluster in affinity group", "id")
	}
	enforcing, ok := sdkObject.Enforcing()
	if !ok {
		return nil, newFieldNotFound("affinity group", "enforcing")
	}
	priority, ok := sdkObject.Priority()
	if !ok {
		return nil, newFieldNotFound("affinity group", "priority")
	}
	hostsRule, ok := sdkObject.HostsRule()
	if !ok {
		return nil, newFieldNotFound("affinity group", "hosts rule")
	}
	convertedSDKHostsRule, err := convertSDKAffinityRule(hostsRule, o)
	if err != nil {
		return nil, err
	}
	vmsRule, ok := sdkObject.VmsRule()
	if !ok {
		return nil, newFieldNotFound("affinity group", "VMs rule")
	}
	convertedSDKVMSRule, err := convertSDKAffinityRule(vmsRule, o)
	if err != nil {
		return nil, err
	}
	vmsList, ok := sdkObject.Vms()
	if !ok {
		return nil, newFieldNotFound("affinity group", "VMs list")
	}
	var convertedVMIDs []string
	for _, vm := range vmsList.Slice() {
		vmid, ok := vm.Id()
		if !ok {
			return nil, newFieldNotFound("VM on affinity group", "id")
		}
		convertedVMIDs = append(convertedVMIDs, vmid)
	}
	...
	return &affinityGroup{
		...
		priority:  AffinityGroupPriority(priority),
		enforcing: enforcing,
		hostsRule: convertedSDKHostsRule,
		vmsRule:   convertedSDKVMSRule,
		vmids:     convertedVMIDs,
	}, nil
}

func convertSDKAffinityRule(sdk *ovirtsdk.AffinityRule, _ *oVirtClient) (*affinityRule, error) {
